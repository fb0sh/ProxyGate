...
| 优先级 | 改动 | 依据 |
| --- | --- | --- |
| ~~P0~~ | ~~`ArcSwap` 快照 + 运行事实用原子量~~ **已做** | 5,000 条时服务端每次发放 12.5ms → 0.110ms，QPS 634 → 2,332 |
| P0 | 每代理退避重探（`next_check_at`），只对可用代理保持高频 | 99.1% 的探测注定失败；每 5 分钟把 1,019 条全探一遍，其中只有 9 条可能成功 |
| P1 | 落盘挪到 `spawn_blocking` / 专用写盘任务 | 同步 `std::fs::write` 跑在 `/get` 的异步任务里 |
| P1 | 采样打分（K=32）替代全量筛选 | 现在每次发放只需 0.110ms，收益已不如从前；等换成 wrk 量出真瓶颈再说 |
