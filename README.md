...
| `fetch(url, headers)` | 第二个参数可选，是一个 `{ ["头名"] = "值" }` 的表 |
...
有些站点会按 IP 限流甚至直接封 IP（示例里的 zdaye 就是这样，抓十来个请求就 405）。
所以每个订阅源都能声明 `via`：

| `via` | 行为 |
| --- | --- |
| `direct` | 永远直连。有些来源不该看到代理的 IP。 |
| `pool` | 优先借**池子里的健康代理**出门；池子空着（冷启动第一次刷新）时退回直连。被站方拉黑的就是代理的 IP，不是你机器的。 |
| `fallback`（默认） | 先直连；被拦/超时/5xx 之后再借一个健康代理重试。一次回退成功之后，这一轮剩下的请求直接走代理，不再白试直连。 |

借代理时最多换**三个不同的代理**，都试完才回到直连：免费代理本身就经常是坏的，
试一次就放弃等于白借。

另外，所有出站请求（抓订阅源、健康探测）都带一个**普通桌面浏览器的 User-Agent**
（取自内置的 UA 池），而不是 `proxygate/x.y.z`——拿自己的名号去敲门，不少站点会
直接拒掉，而我们的目的只是取一份公开的代理列表。脚本还能用
`fetch(url, { ["Referer"] = "..." })` 自己补请求头。
